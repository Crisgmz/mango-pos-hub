...
|------|---------|-------------|
| `/` | `src/pages/Index.tsx` | Dashboard principal con widgets de bienvenida, acciones rápidas, gráfico de ventas, mesas activas |
| `/ventas` | `src/pages/Ventas.tsx` | Punto de venta con sidebar de modos y grid de mesas |
| `/ventas?mode=rapida` | Renderizado por `TablesGrid.tsx` → `QuickSaleScreen.tsx` | Venta rápida sin mesa |
| `/ventas?mode=manual` | Renderizado por `TablesGrid.tsx` → `ManualSaleScreen.tsx` | Venta manual con asignación de mesa + propina 10% |
| `/ventas?mode=delivery` | Renderizado por `TablesGrid.tsx` (sin pantalla dedicada) | Solo muestra grid de mesas, no hay lógica Delivery |
| `/ventas?mode=selfservice` | Renderizado por `TablesGrid.tsx` (sin pantalla dedicada) | Solo muestra grid de mesas, no hay lógica Self Service |
| `/caja` | `src/pages/Caja.tsx` | Gestión de caja con apertura/cierre, movimientos mock |
| `/cocina` | `src/pages/Cocina.tsx` | KDS con comandas mock, estados waiting/preparing |
| `/clientes` | `src/pages/Clientes.tsx` | Lista de clientes mock con búsqueda |
| `/productos` | `src/pages/Productos.tsx` | CRUD de productos con ProductsContext |
| `/reportes` | `src/pages/Reportes.tsx` | Hub de reportes con stats hardcodeados |
| `/ajustes` | `src/pages/Ajustes.tsx` | Menú central con 16 secciones y ~70 items de configuración |
...
| Provider | Archivo | Estado que maneja |
|----------|---------|-------------------|
| `PermissionsProvider` | `src/contexts/PermissionsContext.tsx` | Auth (isAuthenticated, currentUser), roles, permisos |
| `ProductAvailabilityProvider` | `src/contexts/ProductAvailabilityContext.tsx` | Productos agotados (stock out) en memoria |
| `ProductsProvider` | `src/contexts/ProductsContext.tsx` | CRUD de productos, categorías, menús (estado local) |
...
## 9. SERVICIOS / INTEGRACIONES

**No existe ningún servicio, API client, ni integración externa.**

- Sin backend conectado
- Sin Supabase/Lovable Cloud
- Sin localStorage (todo es volátil)
- Sin fetch/axios calls
- Sin WebSocket
- Sin servicio de impresión real
