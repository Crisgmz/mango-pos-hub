...
| Base de datos | ❌ No |
| Autenticación real | ❌ No (PIN mock contra array) |
| Persistencia de datos | ❌ No (todo volátil en useState) |
| API calls | ❌ No (cero fetch/axios) |
| Real-time | ❌ No |
| Impresión | ❌ No (solo toast) |
| Facturación fiscal | ❌ No (NCF mencionado, no implementado) |
| Multi-sucursal | ❌ No (UI existe, sin filtro) |
| Multi-moneda | ❌ No (todo en RD$) |

**Conclusión:** El frontend es un prototipo funcional de UI completo con ~70 pantallas, pero sin ninguna conexión a backend. Toda la lógica de negocio crítica (persistencia, autenticación, facturación, inventario real) requiere implementación backend.
