...
|-------|---------------|---------|
| Login por PIN de 4 dígitos | Match contra array de usuarios mock | `PinLogin.tsx` |
| 6 roles predefinidos | Administrador, Supervisor, Cajero, Mesero, Cocina, Delivery | `users.ts` |
| Administrador tiene todos los permisos | `if (currentRole === "Administrador") return true` | `PermissionsContext.tsx` |
| Acceso a mesa de otro mesero requiere PIN | Si `waiterId !== currentUser.id` y no es Admin/Supervisor | `TablesGrid.tsx` |
| Navegación bloqueada por permisos | Items del nav se deshabilitan visualmente | `TopNavigation.tsx` |
| Cambio de rol en runtime (demo) | Dropdown en TopNavigation permite cambiar rol sin re-login | `TopNavigation.tsx` |
...
| Regla | Implementación |
|-------|---------------|
| Efectivo: monto recibido >= total para confirmar | `PaymentModal.tsx` → `canPay` |
| Tarjeta/Transferencia: permite pago parcial | `PaymentModal.tsx` → `isPartialPayment` |
| Cambio = monto recibido - total (solo efectivo) | `PaymentModal.tsx` |
| Post-pago: generar factura → liberar mesa | `OrderScreen.tsx` → `handleInvoiceClose()` |
...
### Mesa (`Table.status`)
```
disponible → ocupado (al hacer click) → pagando (definido pero no usado) → disponible (al pagar)
```
**Nota:** El estado `pagando` está definido en el tipo pero nunca se asigna en el código.

### Orden (`Order.status`)
```
pending → sent (al "enviar a cocina") → paid (al confirmar pago)
```
**Nota:** Los estados están definidos en el tipo `Order` pero la entidad `Order` nunca se instancia formalmente. El carrito no se convierte en Order.
